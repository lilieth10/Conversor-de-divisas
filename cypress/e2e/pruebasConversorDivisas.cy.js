...
context('Conversor de divisas', ()=>{
  ...
  describe('Revision de la interfaz',()=>{
    ...
    it('Se asegura que existan botones', () => {
        cy.get('.texto-div').find("#boton-aceptar").should('exist'); // Verifica que el botón aceptar exista
        cy.get('.texto-div').find('#mostrar-monedas').should('exist'); // Verifica que el botón mostrar monedas exista
      });
  ...
  describe('Comprueba la situacion en la que no hay fecha',()=>{
    ...
    it('Quita el error',()=>{
      cy.get('#boton-aceptar').click();
      cy.get('#fecha-elegida').type('2024-03-21')
      cy.get('#boton-aceptar').click();
